...
const Tarefa = ({descricao: descricaoOriginal, prioridade, status, titulo, id}: Props) => {
    ...
    return (<S.Card>
        ...
        <S.Descricao disabled={!estaEditando} onChange={evento => setDescricao(evento.target.value)} value={descricao}></S.Descricao>
        <S.BarraAcoes>
            {estaEditando ? (
                <>
                <S.BotaoSalvar onClick={() => {dispatch(editar({
                    descricao, prioridade, status, titulo, id
                }))
                    setEstaEditando(false)
